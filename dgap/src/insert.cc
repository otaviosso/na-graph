...
class BuilderBase {
 ...
 public:
  ...
  CSRGraph<NodeID_, DestID_, invert> MakeGraph(int iters) {
    ...
    if (file_exists(cli_.dbfilename().c_str()) == 0) {
      ...
      std::sort(el.begin(), el.end(), [](const Edge &a, const Edge &b) {
        return a.u < b.u;
      });
    }
...
//      #pragma omp parallel for
      ...
      for (uint64_t i = 0; i < 1000000; i += 1) {
		random_num = rand() % dynamic_edges + 1;
        g.insert(el[random_num].u, el[random_num].v.v);
        if (symmetrize_) {
          g.insert(el[random_num].v.v, el[random_num].u);
        }
//        if(i && i % 10000000 == 0) cout << "inserted " << (i/1000000) << " M dynamic edges" << endl;
		printf("inserted!\n");
      }
      t.Stop();
      cout << "Insertion Time: " << t.Seconds() << " seconds." << endl;
    }
...
typedef NodeWeight<NodeID> WNode;
...
int main(int argc, char* argv[]) {
	CLInsert cli(argc, argv, "insertion", 10);
	if (!cli.ParseArgs())
		return -1;
	WeightedBuilder b(cli);
    WGraph g = b.MakeGraph(cli.iters());
	return 0;
}
