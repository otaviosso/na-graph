...
public:
  ...
  CSRGraph(const char *file, const EdgeList &edge_list, bool is_directed, int64_t n_edges, int64_t n_vertices) {
    ...
    if (RP_init(heap, DB_POOL_SIZE) == 0) {
          is_new = true;
    } 
    ...
    if (is_new) {
      ...
      if ((edges_pm = (DestID_ *) RP_malloc(elem_capacity * sizeof(DestID_))) == NULL) {
  ...
  void resize_V1() { 
    ...
    DestID_* new_edges_ = NULL;
    if ((new_edges_ = (DestID_*) RP_malloc(elem_capacity * sizeof(DestID_))) == NULL) {
      fprintf(stderr, "[%s]: FATAL: edge array allocation failed\n", __func__);
      abort();
    }
