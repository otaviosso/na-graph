...
XPGraph::XPGraph(int argc, const char ** argv){
    ...
    levelgraph = new levelgraph_t(nverts, source, m);
...
vid_t XPGraph::get_source_vertex() {
  return levelgraph->get_source_vertex();
}

index_t XPGraph::get_out_edge_count(){
  return levelgraph->get_out_edge_count();
}

index_t XPGraph::get_in_edge_count(){
  return levelgraph->get_in_edge_count();
}
